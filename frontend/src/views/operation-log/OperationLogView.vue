...
const loading=ref(false),logs=ref<OperationLog[]>([]),users=ref<User[]>([]),total=ref(0),detailVisible=ref(false),selected=ref<OperationLog>()
...
onMounted(async()=>{users.value=(await getUsers({page:1,page_size:200})).items;await load()})
</script>
