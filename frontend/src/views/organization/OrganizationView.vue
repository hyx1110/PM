...
import { createDepartment, createOrganization, getDepartments, getOrganizationTree, updateDepartment, updateOrganization } from '@/api/organization'
...
const departmentForm = reactive({ code: '', name: '', manager_id: undefined as number | undefined, status: 'active' })
const organizationForm = reactive({ department_id: 0, parent_id: undefined as number | undefined, code: '', name: '', level: 'L1' as OrganizationNode['level'], manager_id: undefined as number | undefined, status: 'active' })
...
async function load() {
  ...
  try {
    ...
    if (!activeDepartment.value && departments.value.length) activeDepartment.value = departments.value[0].id
...
function editDepartment(item?: Department) {
  ...
  Object.assign(departmentForm, item ? { code: item.code, name: item.name, manager_id: item.manager_id, status: item.status } : { code: '', name: '', manager_id: undefined, status: 'active' })
...
function editOrganization(item?: OrganizationNode, parent?: OrganizationNode) {
  ...
  Object.assign(organizationForm, item ? {
    department_id: item.department_id, parent_id: item.parent_id, code: item.code, name: item.name,
    level: item.level, manager_id: item.manager_id, status: item.status,
  } : {
    department_id: activeDepartment.value || 0, parent_id: parent?.id, code: '', name: '',
    level: parent ? (`L${Math.min(Number(parent.level.slice(1)) + 1, 4)}` as OrganizationNode['level']) : 'L1', manager_id: undefined, status: 'active',
  })
  ...
  <div class="page-shell">
    ...
    <section class="org-layout">
      <aside class="surface departments">
        <h3>部门</h3>
        <button v-for="item in departments" :key="item.id" :class="{ active: activeDepartment===item.id }" @click="switchDepartment(item.id)">
          <span><strong>{{ item.name }}</strong><small>{{ item.code }}</small></span>
          <el-button v-if="userStore.hasPermission('organization:edit')" link size="small" @click.stop="editDepartment(item)">编辑</el-button>
        </button>
    ...
    <el-dialog v-model="departmentDialog" :title="departmentId?'编辑部门':'新增部门'" width="500px">
      <el-form ref="formRef" :model="departmentForm" :rules="rules" label-position="top"><el-form-item label="部门编码" prop="code"><el-input v-model="departmentForm.code" :disabled="Boolean(departmentId)" /></el-form-item><el-form-item label="部门名称" prop="name"><el-input v-model="departmentForm.name" /></el-form-item><el-form-item label="部门主管"><el-select v-model="departmentForm.manager_id" clearable filterable style="width:100%"><el-option v-for="item in users" :key="item.id" :label="item.name" :value="item.id" /></el-select></el-form-item></el-form>
...
.departments > button { display:flex;width:100%;align-items:center;justify-content:space-between;border:0;border-radius:10px;background:transparent;padding:11px 12px;text-align:left;cursor:pointer; }
.departments > button.active { background:#edf3f9; }
