...
const emptyForm=():ExecutionPayload=>({task_id:0,user_id:userStore.profile?.id,actual_start:beijingNow().format('YYYY-MM-DD'),actual_end:undefined,actual_hours:undefined,status:'running',description:''})
...
function openEdit(row:Execution){editingId.value=row.id;Object.assign(form,{task_id:row.task_id,user_id:row.user_id,actual_start:row.actual_start,actual_end:row.actual_end,actual_hours:Number(row.actual_hours),status:row.status,description:row.description||''});dialogVisible.value=true}
    ...
    <el-dialog v-model="dialogVisible" :title="editingId?'编辑执行记录':'填写执行记录'" width="650px">
      <el-form ref="formRef" :model="form" :rules="rules" label-position="top">
        ...
        <div class="form-grid">
          ...
          <el-form-item v-if="hasGlobalAccess && !editingId" label="执行人" required><el-select v-model="form.user_id" filterable style="width:100%" placeholder="请选择任务项目成员"><el-option v-for="item in executorOptions" :key="item.id" :label="item.name" :value="item.id"/></el-select></el-form-item>
