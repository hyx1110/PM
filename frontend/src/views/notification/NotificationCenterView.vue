...
import { getNotificationPreference, getNotifications, markAllNotificationsRead, markNotificationRead, updateNotificationPreference } from '@/api/notification'
...
async function read(item: AppNotification) { if (item.status === 'unread') await markNotificationRead(item.id); item.status = 'read' }
async function readAll() { const result = await markAllNotificationsRead(); ElMessage.success(`已标记 ${result.updated} 条通知`); await load() }
...
<template>
  <div class="page-shell">
    <header class="page-header"><div><h1 class="page-title">通知中心</h1><p class="page-subtitle">排期新增/变更、确认提醒、临期任务、延期和冲突消息统一归档。</p></div><div><el-button @click="openPreference">通知偏好</el-button><el-button type="primary" plain @click="readAll">全部已读</el-button></div></header>
    ...
    <section class="surface notifications" v-loading="loading">
      <button v-for="item in rows" :key="item.id" class="notification" :class="[{ unread: item.status==='unread' }, levelClass(item.level)]" @click="read(item)">
        <span class="indicator"></span><span class="message"><strong>{{ item.title }}</strong><span>{{ item.content }}</span><small>{{ formatDateTime(item.created_at) }} · {{ (item.delivered_channels || []).join(' / ') || '待投递' }}</small></span><el-tag v-if="item.status==='unread'" size="small">未读</el-tag>
      </button>
