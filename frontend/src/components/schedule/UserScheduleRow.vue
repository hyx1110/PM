...
const emit=defineEmits<{blank:[payload:{userId:number;date:string;hour:number}];booking:[item:Schedule];dropBooking:[payload:{scheduleId:number;userId:number;date:string;hour:number}]}>()
...
function startDrag(event:DragEvent,item:Schedule){event.dataTransfer?.setData('schedule-id',String(item.id));if(event.dataTransfer)event.dataTransfer.effectAllowed='move'}
function dropBooking(event:DragEvent,date:string,hour:number){const scheduleId=Number(event.dataTransfer?.getData('schedule-id'));if(scheduleId)emit('dropBooking',{scheduleId,userId:props.userId,date,hour})}
</script>
...
<template>
  <div class="schedule-row">
    ...
    <div class="timeline" :style="{width:`${gridWidth}px`,gridTemplateColumns:`repeat(${days.length*hours.length},${cellWidth}px)`}">
      <template v-for="day in days" :key="day"><button v-for="hour in hours" :key="`${day}-${hour}`" class="slot" :title="`${day} ${hour}:00`" @click="emit('blank',{userId,date:day,hour})" @dragover.prevent @drop.prevent="dropBooking($event,date,hour)"></button></template>
      <button v-for="entry in visibleSchedules" :key="entry.item.id" class="booking" :class="[typeClass(entry.item.task_type),{conflict:entry.item.has_conflict}]" :style="{left:`${entry.left+2}px`,width:`${entry.width}px`}" :title="`${entry.item.project_name} / ${entry.item.task_name}`" draggable="true" @dragstart="startDrag($event,entry.item)" @click.stop="emit('booking',entry.item)"><strong>{{entry.item.task_name}}</strong><span>{{dayjs(entry.item.start_time).format('HH:mm')}}–{{dayjs(entry.item.end_time).format('HH:mm')}}</span></button>
    </div>
  </div>
</template>
