...
<template>
  <div class="schedule-row">
    ...
    <div class="timeline" :style="{width:`${gridWidth}px`,gridTemplateColumns:`repeat(${days.length*slots.length},${cellWidth}px)`}">
      <template v-for="day in days" :key="day"><button v-for="(slot,index) in slots" :key="`${day}-${slot}`" class="slot" :class="[`slot-${slotKind(day,slot)}`,{'day-boundary':index===0}]" :title="slotTitle(day,slot,index)" :disabled="isUnavailable(day,slot)" @click="selectSlot(day,slot)" @dragover="allowDrop($event,day,slot)" @drop="dropBooking($event,day,slot)"></button></template>
      <button v-for="entry in visibleSchedules" :key="entry.item.id" class="booking" :class="[typeClass(entry.item.task_type),`status-${entry.item.status}`,{conflict:entry.item.has_conflict}]" :style="{left:`${entry.left+2}px`,width:`${entry.width}px`,...projectStyle(entry.item)}" :title="`${entry.item.project_name} / ${entry.item.task_name}`" :draggable="canDrag(entry.item)" @dragstart="startDrag($event,entry.item)" @click.stop="emit('booking',entry.item)"><strong>{{entry.item.task_name}}</strong><span>{{dayjs(entry.item.start_time).format('HH:mm')}}–{{dayjs(entry.item.end_time).format('HH:mm')}}</span></button>
