...
<template>
  <div class="schedule-row">
    ...
    <div class="timeline" :style="{width:`${gridWidth}px`,gridTemplateColumns:`repeat(${days.length*slots.length},${cellWidth}px)`}">
      ...
      <button v-for="entry in visibleSchedules" :key="entry.item.id" class="booking" :class="[typeClass(entry.item.task_type),`status-${entry.item.status}`,{conflict:entry.item.has_conflict}]" :style="{left:`${entry.left+2}px`,width:`${entry.width}px`}" :title="`${entry.item.project_name} / ${entry.item.task_name}`" :draggable="canDrag(entry.item)" @dragstart="startDrag($event,entry.item)" @click.stop="emit('booking',entry.item)"><strong>{{entry.item.task_name}}</strong><span>{{dayjs(entry.item.start_time).format('HH:mm')}}–{{dayjs(entry.item.end_time).format('HH:mm')}}</span></button>
...
.booking.status-pending,.booking.status-changed{border-color:#e4bd70;background:#fff0d4;color:#795518}.booking.status-confirmed,.booking.status-running,.booking.status-completed{border-color:#a9cfb7;background:#dff1e6;color:#356149}.booking.conflict{border:2px solid #c75b5b;background:#f7dddd;color:#913e3e}
</style>
