...
- 个人时间安排：每个登录用户都可为自己设置培训、会议、休假或其他安排，并可主动撤回；生效中的个人安排会显示在共享看板，并阻止项目经理、L3/L4 等任何人员重复预约该时段。
...
升级前请备份数据库与环境文件。不要修改或删除 `20260909_0001` 基线迁移；依次执行 `20260910_0002`、`20260911_0003`、`20260911_0004`、`20260912_0005` 和 `20260914_0006` 增量迁移。`0004` 新增项目审批、项目工时额度、追加工时申请和工作日历，并内置 2026 年法定节假日安排；`0005` 新增个人时间安排表；`0006` 增加员工号、人员档案以及系统角色的人工/职级自动来源。
...
8. 每个用户可在“任务共享看板 → 我的时间安排”中设置自己的培训、会议、休假或其他安排。个人安排同样遵循工作日和上午/下午时段规则；生效期间任何人都不能在重叠时段预约该用户。本人撤回后，该时段立即重新开放预约。
...
- 用户、项目、任务分别通过员工号、项目编号及其关联编码解析关系。项目模板要求部门编码和项目总工时，且只能由模板中填写的项目经理本人导入；导入后仍需 L3 审批。
...
详细清单见 [docs/API.md](docs/API.md)，数据库见 [docs/DATABASE.md](docs/DATABASE.md)，人员模型映射见 [docs/HR_USER_MODEL.md](docs/HR_USER_MODEL.md)，架构见 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)，V2 验收范围见 [docs/V2_SCOPE.md](docs/V2_SCOPE.md)，新增与修改文件见 [docs/V2_FILE_MANIFEST.md](docs/V2_FILE_MANIFEST.md)。
...
建议按“V1 回归 → 数据迁移 → L3/L4 与新权限 → 部门 L3 配置 → 项目审批 → 追加工时 → 个人时间设置/撤回与预约拦截 → 本人审批/撤回预约 → 工作日历与时段边界 → 驾驶舱 → 风险和通知 → Excel → Worker/Beat”的顺序验收。
