...
- 为 L3 补齐评价权限，为 L4 补齐本人项目维护、预约和评价所需入口权限；
- 移除普通项目成员的项目编辑权限；
...
10. 分别用项目经理、L3、L4 和普通成员核对共享看板：项目经理仅见自己的项目组，L3 见全部人员，L4 见本人及全部层级下属，普通成员只见本人。
11. 核对项目/任务范围：L3/超管全量，L4 为本人和全部下属相关数据，项目经理/成员为本人负责或参与的数据；成员身份只读。
...
代码可通过版本控制回退；`0012` 不自动回滚已修复的任务状态或权限数据，数据级回退应恢复升级前备份；`0011` 降级只会把日期列恢复为 `DATETIME` 类型，已被截去的历史时分秒无法恢复；`0010` 降级会删除多人负责人关系，但保留 `tasks.owner_id` 兼容负责人；`0008` 的降级为保护合法成员关系不会删除已回填的项目经理成员行。`0009` 的降级只会重建空的旧人员档案结构，无法恢复升级时删除的人事数据。只有在确认备份可用、相关数据可丢弃时才执行。
