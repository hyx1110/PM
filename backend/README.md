...
- 项目列表只返回当前用户作为项目经理的项目；任务列表只返回当前用户作为负责人的任务；执行记录只返回并只允许维护本人记录。项目审批人通过独立待审批接口处理待办，不会扩大常规项目列表。
...
- 执行记录状态会自动汇总任务、父任务及项目状态；任务管理仅保留手动取消入口，不能绕过执行记录直接改为进行中或已完成。
- 项目经理提交首条任务评价后，项目进入评价锁定阶段；执行记录不能再新增、修改或删除，避免任务或项目状态回退后遗留失效评价。
