...
def get_evaluation(db: Session, project_id: int, user: User) -> dict | None:
    project = assert_project_visible(db, project_id, user)
    row = db.execute(
        select(ProjectEvaluation, User.name.label("evaluator_name"))
        .join(User, User.id == ProjectEvaluation.evaluator_id)
        .where(ProjectEvaluation.project_id == project.id)
    ).first()
    if not row:
        return None
    item, evaluator_name = row
    return {
        **{col.name: getattr(item, col.name) for col in ProjectEvaluation.__table__.columns},
        "evaluator_name": evaluator_name,
    }


def upsert_evaluation(db: Session, project_id: int, payload: EvaluationUpsert, user: User) -> ProjectEvaluation:
    project = db.scalar(
        select(Project)
    ...
    if not roles & {"super_admin", "department_manager"} and project.manager_id != user.id:
        raise forbidden("只有超级管理员、部门主管或本项目经理可以进行项目评价")
    if project.status != "completed":
        raise bad_request("项目执行完成后才能进行评价")
    evaluation = db.scalar(
        select(ProjectEvaluation).where(ProjectEvaluation.project_id == project_id)
    )
    if evaluation:
        raise bad_request("该项目已完成评价，不能重复评价")
    ...
    log_operation(
        ...
        module="evaluation",
        action="create",
        object_type="project_evaluation",
